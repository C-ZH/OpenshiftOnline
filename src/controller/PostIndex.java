...
public class PostIndex extends HttpServlet {
	...
	protected void processLogin(HttpServletRequest request,
		...
		for(int i = 0; i < 5; i ++){
			...
			if(post_tag[post_count - (page_num * 5 + i) - 1].equals("生活")){
				post_tagCode_index[i] = "life";
			}else if(post_tag[post_count - (page_num * 5 + i) - 1].equals("同志社")){
				post_tagCode_index[i] = "school";
			}else if(post_tag[post_count - (page_num * 5 + i) - 1].equals("旅行")){
